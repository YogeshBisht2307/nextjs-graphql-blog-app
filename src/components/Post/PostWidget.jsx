...
const PostWidget = ({ slug, categories }) => {
  ...
  return (
    <div className="bg-slate-800 shadow-lg rounded-lg p-4 lg:p-8 md:p-8 pb-12 mb-8">
      <h3 className="text-xl mb-8 font-semibold text-gray-200 border-b pb-4">{slug ? 'Related Posts' : 'Recent Posts'}</h3>
      {relatedPosts.map((post, index) => (
        <div key={index} className="flex items-center w-full mb-4">
          <div className="w-10 lg:w-16 md:w-16 flex-none">
            <img
              alt={post.title}
          ...
          <div className="flex-grow ml-4">
            ...
            <Link href={`/post/${post.slug}`} key={index}><span className='cursor-pointer text-gray-200 hover:text-green-300 text-xs lg:text-sm md:text-sm'>{post.title}</span></Link>
          </div>
        </div>
      ))}
    </div>
